...
export function RightAxisTicks({ ticks, domain, reversed = false, className = "", formatTick = String }: Props) {
  const span = Math.max(1, domain[1] - domain[0]);

  return <div className={`chart-right-axis ${className}`.trim()} aria-hidden="true">
    {ticks.map((tick) => {
...
export function TimeAxisTick({ x = 0, y = 0, index = 0, visibleTicksCount = 0, payload }: TimeAxisTickProps) {
  if (!payload) return null;
  const textAnchor = index === 0
    ? "start"
    : index === visibleTicksCount - 1 ? "end" : "middle";
