...
import { positionHeatmapTooltip, positionHeatmapTooltipAtDataPoint, tetherHorizontalOverlay } from "./tooltipPosition";
...
describe("positionHeatmapTooltipAtDataPoint", () => {
  ...
  it("repositions a selected data point when the visible domain changes", () => {
    const original = positionHeatmapTooltipAtDataPoint(70, 12, [0, 100], 224, 110, 600, 300, plot);
    const panned = positionHeatmapTooltipAtDataPoint(70, 12, [25, 125], 224, 110, 600, 300, plot);

    expect(original).toMatchObject({ left: 133, top: 75, side: "left", anchorOffset: 55 });
    expect(panned).toMatchObject({ left: 252.5, top: 75, side: "right", anchorOffset: 55 });
  });

  it("lets the tooltip travel beyond the plot edge with its selected data point", () => {
    expect(positionHeatmapTooltipAtDataPoint(20, 12, [25, 125], 224, 110, 600, 300, plot))
      .toMatchObject({ left: -12.5, side: "right" });
    expect(positionHeatmapTooltipAtDataPoint(130, 12, [25, 125], 224, 110, 600, 300, plot))
