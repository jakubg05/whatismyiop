...
describe("trend estimates", () => {
  ...
  it("keeps bracketing estimates when zoomed inside a single sampled segment", () => {
    ...
    expect(
      trendEstimatesForDomain(estimates, 145, 155).map(
        (estimate) => estimate.time,
      ),
    ).toEqual([100, 200]);
    expect(
      trendEstimatesForDomain(estimates, 95, 205).map(
        (estimate) => estimate.time,
      ),
  ...
  it("uses one aggregate value per session", () => {
    ...
    expect(trend[0].estimates.at(-1)!.iop).toBeGreaterThan(
      trend[0].estimates[0].iop,
    );
  });
  ...
  it("preserves a linear series and returns finite LOWESS uncertainty estimates", () => {
    const measurements = Array.from({ length: 20 }, (_, index) =>
      reading(index, 12, "OD", 18 + index * 0.05),
    );
    const estimates = buildTrendSeries(measurements, "sessions", "median")[0]
      .estimates;
    ...
    expect(
      estimates.every(
        (estimate) =>
          Number.isFinite(estimate.lower) && Number.isFinite(estimate.upper),
      ),
    ).toBe(true);
  });
  ...
  it("does not use measurement position in the trend", () => {
    ...
    const withoutPositions = measurements.map((measurement) => ({
      ...measurement,
      position: "",
    }));

    const withPositionTrend = buildTrendSeries(
      measurements,
      "sessions",
      "median",
    )[0].estimates;
    const withoutPositionTrend = buildTrendSeries(
      withoutPositions,
      "sessions",
      "median",
    )[0].estimates;
