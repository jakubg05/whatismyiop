...
import type { DiurnalPoint, DiurnalYAxisScale } from "./comparisonDiurnalData";
import { EyeToggleGroup } from "../../../../shared/ui";
import {
  MeasurementViewControl,
  SeriesVisibilityControl,
  TargetControl,
  TargetLineOverlay,
} from "../../chronological/chart/ChartControls";
import { CHART_PLOT_LEFT, CHART_PLOT_RIGHT } from "../../shared/chartLayout";
import { RightAxisTicks } from "../../chronological/chart/RightAxisTicks";
import {
  DIURNAL_BIN_COUNT,
  MINUTES_PER_BIN,
  diurnalBinRange,
...
function pinnedTooltipCoordinates(
  x: number,
  y: number,
  ...
  let left =
    x + DIURNAL_TOOLTIP_GAP + DIURNAL_TOOLTIP_WIDTH <=
    chartWidth - DIURNAL_TOOLTIP_INSET
  ...
  let top = Math.max(
    DIURNAL_TOOLTIP_INSET,
    Math.min(
      y - DIURNAL_TOOLTIP_HEIGHT / 2,
      chartHeight - DIURNAL_TOOLTIP_HEIGHT - DIURNAL_TOOLTIP_INSET,
    ),
  );
  const hoverTooltip = chart?.querySelector<HTMLElement>(
    ".recharts-tooltip-wrapper",
  );
  if (
    chart &&
    hoverTooltip &&
    getComputedStyle(hoverTooltip).visibility !== "hidden"
  ) {
...
function DiurnalTooltip({
  ...
  return (
    <div className="diurnal-tooltip">
      ...
      <span className="diurnal-tooltip__eye">
        <span
          className={`dot dot--${point.eye.toLowerCase()}`}
          aria-hidden="true"
        />
        {eyeName(point.eye)}
      </span>
      ...
      <dl className="diurnal-tooltip__rows">
        <div>
          <dt>Time</dt>
          <dd>{diurnalBinRange(point.bin)}</dd>
        </div>
        <div>
          <dt>SD</dt>
          <dd>{point.sd.toFixed(1)} mmHg</dd>
        </div>
        <div>
          <dt>{measurementView === "raw" ? "Measurements" : "Sessions"}</dt>
          <dd>{point.count}</dd>
        </div>
      </dl>
    </div>
...
export function ComparisonDiurnalChart({
  series,
  yScale,
  targetEnabled,
  targetValue,
  ...
  useEffect(() => {
    ...
    function dismissPinnedTooltip(event: PointerEvent) {
      ...
      if (
        ...
        target.closest(".diurnal-chart .recharts-symbols")
      )
        return;
      setPinnedTooltip(null);
  ...
  if (inactiveState) {
    return (
      <div className="diurnal-chart diurnal-chart--inactive">
    ...
    <>
      <div className="diurnal-chart">
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart
            data={points}
            margin={{ top: 16, right: CHART_PLOT_RIGHT, bottom: 20, left: 0 }}
          >
            ...
            {Array.from(
              ...
              (_, bin) =>
                bin % 2 === 1 && (
                  <ReferenceArea
                    key={bin}
                    x1={bin * MINUTES_PER_BIN}
                    x2={(bin + 1) * MINUTES_PER_BIN}
                    fill="#e8ecee"
                    fillOpacity={0.72}
                    stroke="none"
                  />
            ...
            <XAxis
              ...
              ticks={Array.from(
                { length: DIURNAL_BIN_COUNT },
                (_, bin) => bin * MINUTES_PER_BIN + MINUTES_PER_BIN / 2,
              )}
              tickFormatter={diurnalHourTick}
              minTickGap={18}
            ...
            <Tooltip
              ...
              content={<DiurnalTooltip measurementView={measurementView} />}
              cursor={false}
              isAnimationActive={false}
            />
            {visibleSeries.map((item) => {
              const dimmed =
                focusedSeriesId !== null && focusedSeriesId !== item.id;
              return (
                <Scatter
                  key={item.id}
                  name={item.name}
                  ...
                  onClick={(data: ScatterPointItem, _index, event) => {
                    ...
                    const { left, top } = pinnedTooltipCoordinates(x, y, chart);
                    setHoveredSeriesId(null);
        ...
        <RightAxisTicks
          className="chart-right-axis--diurnal"
          ticks={yScale.ticks}
          domain={yScale.domain}
        />
        {targetEnabled && (
        ...
        {pinnedTooltip && (
          <div
            className="diurnal-tooltip-pinned"
            style={{ left: pinnedTooltip.left, top: pinnedTooltip.top }}
          >
            <DiurnalTooltip
              active
              payload={[{ payload: pinnedTooltip.point }]}
              measurementView={measurementView}
            />
          </div>
        )}
      ...
      />
      <footer
        className="diurnal-controls"
        role="group"
        aria-label="Diurnal chart controls"
      >
        <MeasurementViewControl
        ...
        <EyeToggleGroup
          mode="single"
          label="Eye shown in diurnal chart"
          value={eye}
